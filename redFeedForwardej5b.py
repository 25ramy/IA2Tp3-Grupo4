...
def generar_datos_clasificacion(cantidad_ejemplos, cantidad_clases):
    ...
    xgen = np.linspace(0, 1, cantidad_ejemplos) + 0.15*randomgen.standard_normal(size=cantidad_ejemplos)
    xcorr = np.linspace(0, 1, cantidad_ejemplos) 

    a= random.randrange(0, 5)
    b= random.randrange(0, 5)

  
    for i in range(0, cantidad_ejemplos):
        x[i,0]=i
        x[i,1]=a+b*xgen[i]+a*xgen[i]*xgen[i]+b*xgen[i]*xgen[i]*xgen[i]
        t[i,0]=a+b*xcorr[i]+a*xcorr[i]*xcorr[i]+b*xcorr[i]*xcorr[i]*xcorr[i]
